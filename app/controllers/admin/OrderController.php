...
class OrderController extends AppController{
    ...
    public function indexAction(){
        ...
        $orders = \R::getAll("SELECT `order`.`id`, `order`.`user_id`, 
`order`.`status`,`order`.`date`, `order`.`update_at`,   
`user`.`name`, ROUND(SUM(`order_product`.`sum`), 2) AS `sum` FROM `order`
...
GROUP BY `order`.`id` ORDER BY `order`.`status`, `order`.`id` LIMIT $start, $perpage");
    ...
    public function deleteAction(){
        $order_id = $this->getRequestID();
        $order = \R::load('order', $order_id);
        \R::hunt('order_product', 'order_id = ?', [$order_id]);
        \R::trash($order);
    ...
    public function cancellationAction(){
        ...
        $product = \R::load('order_product', $product_id);
        \R::trash($product);
