...
class Order extends AppModel{
    ...
    public static function saveOrderProduct($order_id){
        ...
        foreach($_SESSION['cart'] as $product_id => $product){
            ...
            $sql_part .= "($order_id, $product_id, {$product['qty']}, '{$product['title']}', {$product['price']}, $sum, '{$product['img']}','{$product['alias']}' ),";
            }
            $sql_part = rtrim($sql_part, ',');
            \R::exec("INSERT INTO order_product (order_id, product_id, qty, title, price, sum, img, alias) VALUES $sql_part");
    ...
    App::$app->getProperty('smtp_port'), App::$app->getProperty('smtp_protocol')))
        ...
        ->setFrom([App::$app->getProperty('smtp_login') => App::$app->getProperty
        ('shop_name')])
        ->setTo($user_email)
